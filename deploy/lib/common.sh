...
deploy_env_key_allowed() {
  case "$1" in
    COMPOSE_PROJECT_NAME|APP_UID|APP_GID|ISSUEBOT_HOME|ISSUEBOT_BIND_ADDRESS|ISSUEBOT_SECRET_ENV|ISSUEBOT_CHECKOUT|ISSUEBOT_BRANCH|ISSUEBOT_STATE_DIR|ISSUEBOT_NATIVE_PROCESS_PATTERN|ISSUEBOT_FIRST_CUTOVER|CODEX_CLI_PROVIDER_IMAGE|CODEX_CLI_PROVIDER_PROTOCOL_VERSION) return 0 ;;
    *) return 1 ;;
  esac
}
