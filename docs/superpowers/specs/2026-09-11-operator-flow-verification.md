...
| Full combined Java `./mvnw -q verify` | **PASS on final source `971b3e9`**: 1,741 tests, zero failures/errors/skips, exit 0. |
| Full combined JavaScript `node --test src/test/js/*.cjs src/test/js/*.js` | **PASS on final source `971b3e9`**: 64 tests, zero failures/skips, exit 0 with loopback permission. |
| Final `git diff --check` | **PASS**, exit 0 after final source fixes and evidence update. |
| Browser: 1440px/390px, light/dark | **PASS** for the inspected screen/state matrix below; all reported visual findings verified fixed. |
| Browser: keyboard, real HTMX history, drafts/disclosures, badge transitions | **PASS**: actual real-HTMX navigation, paged histories, draft/disclosure preservation and fresh retry gates, notification count/unavailability/recovery transitions. Synthetic-state limits remain below. |
| Whole-branch independent review | **PASS after fixes**: whole-branch review identified two Important and three Minor findings; one combined fix wave `971b3e9` and one scoped review addressed all five, with no new blocking issues or deferred findings. |
...
| --- | --- | --- |
| #134 durable actor/action/outcome/rationale; transaction rollback, duplicate delivery, restart, immutable identity | `DecisionHistoryPersistenceTest`, `DecisionProducerIntegrationTest`, approval/dispatch persistence tests | `/issues/1` and `/issues/1/decisions?page=0`, `?page=1`: 31 decisions including Operator, Automation, Actor unavailable. Desktop/phone layout and 25→6-record paging/disclosure checks passed. |
| #134 private structured data, artifact ownership, escaping, deleted/legacy artifacts, paging/empty states | `DecisionHistoryRenderTest`, `DecisionHistoryPersistenceTest` | Empty history `/issues/3`; legacy review `/issues/7`; recoverable not-found `/issues/999999`. Empty history and unknown legacy evidence inspected; plain not-found is a synthetic deleted-item stand-in. |
| #136 normalized verdict/score changes; exact criterion/finding identity, moved lines/severity changes | `ReviewScoreHistoryAssemblerTest`, `IssueDetailPlanReviewRenderTest` | `/issues/1` includes newly met criteria and resolved finding; `?reviewAttempt=1` selects earlier evidence. Browser rendered authoritative 92%/passed verdict, dimension deltas, newly met criteria, and disclosure state. |
...
| #149 `c49c64a60d82be01`: six labels/icons/visible states and actual timing target | `IssueDetailLivePollRenderTest.allSixStagesRenderTheirOwnLabelIconAndVisibleState`, `timingLinkTargetsAnExistingStableElementInRenderedContent` | `/issues/1` and live fragment ready for visual confirmation; original keyed finding is verified; GitHub checklist update occurs after merge. |

## Focused commands and results

### Final release evidence

The final source tree `971b3e9` passed `./mvnw -q verify` (**1,741 tests, zero failures/errors/skips**) and `node --test src/test/js/*.cjs src/test/js/*.js` (**64 passed, zero failed/skipped**), both exit 0. The only subsequent release commit records this documentation, not executable changes. Independent scoped review verified all five final findings addressed; no new breakage or parked findings remain. Baseline Mockito/JVM, Logback and Flyway/H2 notices plus intentionally exercised failure-path logs were reviewed as test-environment/expected noise, not unhandled failures.

Final interactive export `/tmp/issuebot-operator-flow-final-verified` (134 exact routes) was served at loopback port 41786 with opt-in `--interactive`. Actual HTMX GET updates preserved an unsent recovery note and expanded technical/advanced sections while known-unmet state disabled retry and verified-ready state re-enabled it. Notification panel and bell agreed at 2, 0, and 1; synthetic HTTP 503 showed the stale count as `1?` with an unavailable label, then a successful update restored `1`. No actual retry, authentication check, notification-read mutation, or provider invocation occurred.

Computed layout checks at both 390 and 1440 widths verified hidden deep/unknown-context navigation as `display:none`, and exactly the intended first/middle/last links or disabled boundaries. Filtered return retained `q=fixture` and original issue order; returning from the final row restored a visible final row at scroll position 478 in the desktop fixture. Notification default pager reached Page 2 via its actual emitted query, and the Approval search form returned one group. Phone checkbox visual is 16×16px with a 44px label hit area; filters now fill the available width. Needs You displays a neutral `Overall 92%` stat in light and dark themes. Browser viewport override was reset after verification.

The historical failures and initial browser limitations below are retained as an audit trail; they were resolved by the final fix wave and checks above. Exact 30-minute expiry, denied storage, concurrent notification arrivals, server-side authorization and transaction behavior are established by automated tests, not by changing live browser storage or production state.


### Root combined run and browser observations

The first combined Java run at `820e4c6` failed with 4 failures and 26 errors: two persistence slices lacked the newly required prerequisite-service bean; three lifecycle tests still observed untyped notification methods; and the positive correction fixture did not seed the immutable approved-plan identity. Commit `5cf5de7` updated those fixtures without changing runtime guards and added three negative identity cases. All 83 focused tests then passed. The full `./mvnw -q verify` run at `5cf5de7` exited 0 with 1,737 tests, zero failures/errors/skips. Full JavaScript testing exited 0 with 63 passes on the unchanged frontend tree.

Actual in-app-browser checks used a synthetic read-only export, not production:

- At 1440×1000 light: Dashboard, Repositories (expanded full-width lessons), Setup, Settings, and notification panel were visually inspected. At 390×844 dark: issue detail/recovery, legacy review, notification history, Setup, Settings, Costs, Needs You, Approvals, and not-found were inspected. Desktop dark repository/history and phone light issue detail were also inspected. Checked pages had no document-wide horizontal overflow; Setup intentionally provides a named internal table scroll region.
- The dedicated real-HTMX fixture verified mouse, Enter, and Space navigation with matching request URLs and browser URLs, plus native Back/Forward. The combined UI verified `/issues?q=fixture` → issue 4 → Next to issue 3 → Previous to issue 4 → Back to results, retaining the original query and result order.
- Decision history expanded and changed from 25 records on Page 1 to 6 on Page 2 without collapsing. Notification group history loaded events 0–24, then Later events showed 25–30 while remaining expanded. Repository lessons remained expanded across later full-page navigation. Theme persisted when switched using the visible menu control.
- Legacy review explicitly showed unavailable findings/criteria and unknown workflow-run identity rather than inventing zero counts or a comparison. Notification panel and bell agreed on one unread action in the basic snapshot.
- Browser inspection caught authored flex styles overriding native `hidden`, exposing duplicate Previous/Next controls; narrow right-aligned notification filters and oversized checkbox; and a neutral 92% score using a red count badge in Needs You. These are included in the final fix wave. Independent source review additionally found malformed array items could falsely resolve prior findings; safe snapshot-failure diagnostic logging is included as polish.
- The existing export disables live polling (`hx-trigger="none"`), so leaving a draft open does **not** prove update preservation. The final fixture-only manual GET harness will test draft/disclosure preservation alongside fresh eligibility and badge transitions. The notification Next-page link's full query has no current manifest alias; explicit `?page=1` rendered correctly, and exact known aliases are being added. No production endpoint failure is inferred from that fixture limitation.

No source-dependent browser findings are marked resolved until the amended export is inspected. The two suites above are not a claim that the later fix tree is already verified.
