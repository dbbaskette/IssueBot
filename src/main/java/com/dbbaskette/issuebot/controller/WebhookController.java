 ...
 * accelerator.
 * <p>
 * Disabled (503) unless {@code ISSUEBOT_WEBHOOK_SECRET} is set. Every delivery
 * must carry a valid {@code X-Hub-Signature-256} HMAC-SHA256 signature over the
 * raw body, or it is rejected with 401. Once past those two gates, the handler
 * never surfaces an error back to GitHub — any downstream failure (malformed
 * payload, evaluation exception) is logged and swallowed, always answering 204,
 * so GitHub does not disable the webhook after repeated failures.
 */
@RestController
public class WebhookController {
    ...
    private void dispatch(byte[] rawBody, String eventType) throws Exception {
        ...
        if ("labeled".equals(action)) {
            ...
            if (AGENT_READY_LABEL.equals(labelName)) {
                pollingService.evaluateSingleIssueFromWebhook(repo, payload.path("issue"));
            }
