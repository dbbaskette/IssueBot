...
public interface DecompositionGroupRepository extends JpaRepository<DecompositionGroup, Long> {
    ...
    default Optional<DecompositionGroup> findOwningByRepo(Long repoId) {
        return findByRepoIdAndStateIn(repoId, EnumSet.of(
                DecompositionGroupState.CREATING, DecompositionGroupState.ACTIVE,
                DecompositionGroupState.NEEDS_ATTENTION, DecompositionGroupState.COMPLETING,
                DecompositionGroupState.ABANDONING)).stream().findFirst();
    }
}
