...
public final class LogSanitizer {
    ...
    private static final Pattern GENERIC_SECRET = Pattern.compile("((?:token|key|secret|password)\\s*[:=]\\s*)(?!(?:sk-ant-|ghp_|Bearer\\s))\\S+", Pattern.CASE_INSENSITIVE);
    ...
    public static String sanitize(String input) {
        ...
        String result = input;
        result = GENERIC_SECRET.matcher(result).replaceAll("$1***REDACTED***");
        ...
        result = result.replaceAll("(?i)\\S*(?:\\.codex/auth\\.json|\\.ssh/\\S+|/\\.env(?:\\s|$))", "***REDACTED_PATH***");
        return result;
    }
}
