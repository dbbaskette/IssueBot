...
public class IterationManager {
    ...
    public Iteration claimPlanCorrectionIteration(TrackedIssue trackedIssue, int iterationNum) {
        ...
        iteration.setImplementationHarnessId(claimTarget.getResolvedHarnessId());
    ...
    public Iteration claimImplementationIteration(TrackedIssue expected, int number) {
        ...
        iteration.setImplementationHarnessId(issue.getResolvedHarnessId());
    ...
    /** Approved-plan coding stops for an explicit blocker; do not suggest splitting by default. */
    public void handleHarnessBlocked(TrackedIssue trackedIssue, String reason) {
        String comment = "## IssueBot: Coding run needs guidance\n\n"
                + "The coding harness could not complete the assigned requirements:\n\n"
                + "> " + reason.replace("\n", "\n> ") + "\n\n"
                + "IssueBot has not published this blocked coding turn for final review; "
                + "an earlier PR may still be open. "
                + "Inspect the retained branch and turn history, repair the environment or add guidance, "
                + "then use Extend and continue when available, or explicitly start a fresh attempt when ready.\n\n"
                + "---\n*Generated by [IssueBot](https://github.com/dbbaskette/IssueBot)*";
        escalateFailure(trackedIssue, "Coding run blocked", reason,
                "HARNESS_BLOCKED", "Coding run blocked: " + reason, comment);
    }
